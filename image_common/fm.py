...
class FlowMatching(nn.Module):
    ...
    def get_loss(self, x1, class_label=None, x0=None):
        ...
        t_ = t.view(-1, *([1] * (x1.dim() - 1)))  # reshape t for broadcasting
        x_t = (1.0 - t_) * x0 + t_ * x1  # compute interpolated sample
        u_t = x1 - x0  # compute target velocity
        if class_label is not None:
            model_out = self.network(x_t, t, class_label=class_label)
        else:
            model_out = self.network(x_t, t)
        loss = (model_out - u_t).pow(2).mean()  # compute MSE loss
        ######################
    ...
    def sample(
        ...
        for i, t in enumerate(pbar):
            ...
            xt_next = self.fm_scheduler.step(xt, vt, dt)  # perform Euler step
            xt = xt_next # update xt
            ######################
